...
async def adgroups_details_sync(
    shops         = Depends(get_shops),
    seller_id:int = Query(None, description="同步指定店铺, 默认None同步所有店铺"),
    data_at:  str | None = Query(None, description="开始日期 YYYY-MM-DD"),
    data_to:  str | None = Query(None, description="结束日期 YYYY-MM-DD"),
):
    if seller_id is not None and seller_id not in shops:
        raise HTTPException(status_code=404, detail="shop not found")
    ...
    for shop in targets:
        try:
            if data_to is None or data_at is None:
                yesterday = datetime.now(tz = ZoneInfo(shop.timezone)) - timedelta(days=1)
                data_to = yesterday.strftime("%Y-%m-%d")
                data_at = yesterday.strftime("%Y-%m-%d")
