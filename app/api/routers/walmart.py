"""Walmart 路由: 订单/商品/变体/货运/库存/评价/账单/用户 共 8 个资源。

路由结构:
  # Order
  GET  /walmart/shop/{shop_id}/orders/search     — 搜索订单
  GET  /walmart/orders/sync                      — 同步订单
...
# basemodel
from app.api.schemas import ApiResponse, WMOrderSearch, WMOrderSync

# 资源
from app.resources.walmart.order import Order
...
def get_shops(request: Request):
    return request.app.state.walmart_shops


# ═════════════════════════════════════════════════════════
#  Order
# ═════════════════════════════════════════════════════════
@router.get("/walmart/shop/{shop_id}/orders/search", response_model=ApiResponse)
async def order_search(
    shop_id: str = Path(description="SHOP ID 必填"),
    shops:  dict = Depends(get_shops),
    searchmodel: WMOrderSearch = Depends(),
):
    """
    查询并保存指定店铺的订单数据
    """
